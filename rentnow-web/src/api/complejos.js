  ...
  try {
    ...
    const complejos = result.docs.map((complejo) => {
      ...
      return {
        id: complejo.id,
        ...complejoData,
        fechaAlta: complejoData.fechaAlta.toDate(),
      };
    });
    return {
      status: "OK",
...
export async function getAllComplejosApi() {
  try {
    const result = await firebase.firestore().collection("complejos").get();
    const complejos = result.docs.map((complejo) => {
      const complejoData = complejo.data();
      return { id: complejo.id, ...complejoData };
    });
    return {
      status: "OK",
      message: "Se consultaron los complejos con exito",
