...
import { FolderOpenOutlined, FolderSharedOutlined } from '@material-ui/icons';
...
export const ImageUploader = React.memo(function ({ maxFiles, url, getUrls }) {
	...
	const [cantImgSubidas, setCantImgSubidas] = useState(0);
	...
	const handleFileUpload = async () => {
		...
		Promise.all(imgsRefs.map((img) => returnPromise(img))).then(() => {
			setUploading(false);
			setUploaded(true);
			setCantImgSubidas(imgsRefs.length);
			setImgs([]);
		});
	};
			...
			<Grid container direction="column" alignContent="center" alignItems="center" justify="center">
				...
				<Grid item>
					<Typography variant="h4">
						{nroImagen} de {cantImgSubidas} imágenes subidas con éxito.
					</Typography>
				</Grid>
			</Grid>
		);
	...
	return (
		<>
			<FilePond
				...
				labelIdle={'Suelte las imágenes aqui o <span class="filepond--label-action"> búsquelas </span>'}
				acceptedFileTypes={['image/png', 'image/jpeg']}
				...
				onremovefile={async (error, fileRemoved) => {
					setImgs((oldImgs) => {
						const array = oldImgs.filter((el) => el.name !== fileRemoved.file.name);
						return array;
					});
				}}
			/>
			{imgsRefs.length === 0 ? null : (
				<Button onClick={handleFileUpload} color="primary" variant="contained" fullWidth>
					Subir Imágenes
				</Button>
			)}
		</>
